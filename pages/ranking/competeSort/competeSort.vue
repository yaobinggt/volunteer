<template>
	<view>
		...
		<rankScore :collegeList="collegeList"></rankScore>
	</view>
</template>
	...
	export default {
		...
		methods:{
			getList(){
				...
				this.$api.getCompete(data).then(data=>{
					...
					//转换特色数据
					if (data.search.famous_list.length > 1) {
						let values = data.search.famous_list
						for (var i = 0; i < values.length; i++) {
							var obj = {
								label: values[i].famous_name,
								value: values[i].famous_id
							};
					...
					console.log(this.districtList,this.famousList,this.levelList)
				})
			},
			getListTwo(){
				let data= Object.assign(this.from,{});
